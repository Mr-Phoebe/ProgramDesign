...
class DQN:
    def __init__(self, params):
        ...
        self.x = tf.placeholder(
            'float', [None, params['width'], params['height'], 6],
        ...
        self.b3 = tf.Variable(
            ...
            name=self.network_name + '_' + layer_name + '_biases')
        self.ip3 = tf.add(
            tf.matmul(self.o2_flat, self.w3),
            self.b3,
        ...
        self.Q_pred = tf.reduce_sum(
            tf.multiply(self.y, self.actions), reduction_indices=1)
    ...
    def train(self, bat_s, bat_a, bat_t, bat_n, bat_r):
        ...
        _, cnt, cost = self.sess.run([self.optim, self.global_step, self.cost],
                                     feed_dict=feed_dict)
        return cnt, cost
