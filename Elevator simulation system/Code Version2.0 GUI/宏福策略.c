				...
				int *ifB)
{
	...
	int transform_hongfu(Input*,int,int,int,int[][9],int*,int*,int*,int);
	...
	//hongfu
	if(!floor) *ifB=0;
	if(!transform_hongfu(in,in_size,T,floor,keySitu,&drt,ifB,out,T) && *ifB!=1)/*empty*/{
		*direction=-2;
		out[T+1]=100*out[T];
		return -2;
	}
...
int transform_hongfu(Input *in,int size,int curT,int floor,
	/*as result returned*/int keySitu[][9],int *drt,int *ifB,int out[],int T)
{
