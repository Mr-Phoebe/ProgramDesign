	...
	//返回优先condition方向（1->'u' or -1->'d'）所需的时间
{
	...
	if (condition==1)
	{
		if (max_3(max_in,high_up,high_down) > floor)//需要上行
		{
			time+=max_3(max_in,high_up,high_down)-floor;

			if (min_3(min_in,low_up,low_down) >= floor)//不需要再下行
				return high_down==0?time:time+max_3(max_in,high_up,high_down)-high_down;
			else
			{
				time*=2;//下行到原位置
				time+=floor-min_3(min_in,low_up,low_down);
	...
	{
		if (min_3(min_in,low_up,low_down) < floor)//需要下行
		{
			time+=floor-min_3(min_in,low_up,low_down);

			if (max_3(max_in,high_up,high_down) <= floor)//不需要上行
				return low_up==10?time:time+low_up-min_3(min_in,low_up,low_down);
			else
			{
				time*=2;//上行到原位置
