...
bool Widget::isValidCheckInDate(QString& checkin,int size)
{
    int days = QDate::fromString(checkin, "yyyy-MM-dd").daysTo(thedate);
    days*=-1;
    if( size >= 4)
        return false;
    else if( size >= 3 && days < 30 )
        return false;
    else if( size >= 2 && days < 14 )
        return false;
    else if( size >= 1 && days < 7 )
        return false;
    return true;
}
...
void Widget::on_commitButton_clicked()
{
    ...
    {
        QMessageBox::warning(this, tr("预定管理"), tr("用户信息不完整"));
    }
    else if (items.isEmpty())
    {
        QMessageBox::warning(this, tr("预定管理"), tr("没有选定预定房间"));
    }
    else if ( isValidCheckInDate(checkintime,items.size()) )
    {
        QMessageBox::warning(this, tr("预定管理"), tr("预定入住时间过长"));
    }
    else
    {
        QString roomno;
...
void Widget::updateOrderPage()
{
    ...
    QStringList remove;
    QString sql = "select name, roomnos, checkintime from customer_t";
    QSqlQuery query(sql);
    ...
    while (query.next())
    {
        ...
        QString checkin = query.value(2).toString();
        bool small = QDate::fromString(checkin, "yyyy-MM-dd") < (QDate::currentDate());
        if(small)
        {
            remove << name;
            continue;
    ...
    // ## 查询最多一列的房屋行数
    sql = "select count(*) from room_t where status = 1 group by roomtype";
    query.exec(sql);
    ...
    while (query.next())
    {
        ...
        if (roomtype == 0)
            ...
            ui->orderRoomTableWidget->setItem(r4++, roomtype, new QTableWidgetItem(roomno));
    }
